...
def toast_show(title="Chieri Bot Peek API", msg=""):
    toast.show_toast(title=title, msg=msg,
                     icon_path="Chieri64.ico", duration=3, threaded=True)
...
class SysTrayIcon(object):
    ...
    def __init__(self,
        ...
        menu_options = menu_options + (('切换手机/电脑', None, sstat.change_device),
                                       ('切换公开/隐私', None, sstat.change_public),
                                       ('退出', None, self.QUIT))
...
def start_tray():
    icons = 'Chieri64.ico'
    hover_text = "Chieri Bot Peek API"
    menu_options = (())

    def killself(sysTrayIcon):
        os._exit(0)

    SysTrayIcon(icons, hover_text, menu_options, on_quit=killself, default_menu_index=1)
